...
mLLMCelltype는 단일 세포 RNA 시퀀싱(scRNA-seq) 데이터에서 세포 유형 주석을 위한 반복적 다중 LLM 합의 프레임워크입니다. 여러 대형 언어 모델(OpenAI GPT-5.2/5, Anthropic Claude-4.6/4.5, Google Gemini-3, X.AI Grok-4, DeepSeek-V3, Alibaba Qwen3, Zhipu GLM-4, MiniMax, Stepfun, OpenRouter 등)의 예측 결과를 결합하여, 이 프레임워크는 주석 정확도 향상을 목표로 하면서 생물정보학 및 전산 생물학 연구를 위한 투명한 불확실성 정량화를 제공합니다.
...
**권장 마이그레이션**: 더 나은 성능과 향상된 추론 능력을 위해 `gemini-3-pro` 또는 `gemini-3-flash`를 사용하세요. 별칭 `gemini-1.5-pro`와 `gemini-1.5-flash`는 -002 버전을 가리키므로 2025년 9월 24일까지 계속 작동합니다.

**📢 중요: Claude 모델 지원 중단 (2025-07-21)**

Anthropic은 2025년 7월 21일에 여러 Claude 모델을 중단할 예정입니다:
- **중단될 모델**: Claude 2, Claude 2.1, Claude 3 Sonnet (버전 없음), Claude 3 Opus (버전 없음)

**권장 마이그레이션**:
- Claude 2/2.1 → `claude-sonnet-4-5-20250929` 또는 `claude-3-5-sonnet-20241022`
- Claude 3 Sonnet → `claude-sonnet-4-5-20250929` 또는 `claude-3-7-sonnet-20250219`
- Claude 3 Opus → `claude-sonnet-4-5-20250929` 또는 `claude-3-opus-20240229`

서비스 중단을 피하기 위해 2025년 7월 21일 이전에 모델을 업데이트하시기 바랍니다.

## 주요 기능

- **다중 LLM 합의 아키텍처**: 다양한 LLM의 예측 결과를 결합하여 단일 모델의 한계와 편향을 줄임
...
- **환각 감소**: 모델 간 심의를 통해 비판적 평가를 통해 부정확하거나 근거 없는 예측을 식별하는 데 도움
...
- **OpenAI**: GPT-5.2/GPT-5/GPT-4.1 ([API 키](https://platform.openai.com/settings/organization/billing/overview))
- **Anthropic**: Claude-4.6-Opus/Claude-4.5-Sonnet/Claude-4.5-Haiku ([API 키](https://console.anthropic.com/))
- **Google**: Gemini-3-Pro/Gemini-3-Flash ([API 키](https://ai.google.dev/?authuser=2))
- **Alibaba**: Qwen3-Max ([API 키](https://www.alibabacloud.com/en/product/modelstudio))
- **DeepSeek**: DeepSeek-V3/DeepSeek-R1 ([API 키](https://platform.deepseek.com/usage))
- **Minimax**: MiniMax-M2.1 ([API 키](https://intl.minimaxi.com/user-center/basic-information/interface-key))
- **Stepfun**: Step-3 ([API 키](https://platform.stepfun.com/account-info))
- **Zhipu**: GLM-4.7/GLM-4-Plus ([API 키](https://bigmodel.cn/))
- **X.AI**: Grok-4/Grok-3 ([API 키](https://accounts.x.ai/))
- **OpenRouter**: 단일 API로 여러 모델에 액세스 ([API 키](https://openrouter.ai/keys))
  - OpenAI, Anthropic, Meta, Google, Mistral 등의 모델 지원
  - 형식: 'provider/model-name' (예: 'openai/gpt-5.2', 'anthropic/claude-opus-4.5')
...
**빠른 시작**: 설치 없이 Google Colab에서 mLLMCelltype을 즉시 사용해보세요! 위의 배지를 클릭하여 예제와 단계별 가이드가 포함된 대화형 노트북을 열어보세요.
...
consensus_results = mct.annotate.interactive_consensus_annotation(
    ...
    models=["gpt-5.2", "claude-sonnet-4-5-20250929", "gemini-3-pro"],  # 다양한 대형 언어 모델 사용
    api_keys={                                                     # 각 LLM 제공업체의 API 키 설정
        "openai": "your_openai_api_key",                             # OpenAI GPT-5 API 키
...
1. **Anthropic Claude 모델** (권장)
   - `claude-sonnet-4-5-20250929`
   - `claude-opus-4-1-20250805`

2. **OpenAI 모델**
   - `o1` / `o1-pro` - 고급 추론 능력
   - `gpt-5.2` / `gpt-5` - 다양한 세포 유형에서 강력한 성능
   - `gpt-4.1` - 최신 GPT-4 변형

3. **Google Gemini 모델**
   - `gemini-3-pro` - 향상된 추론으로 최상급 성능
   - `gemini-3-flash` - 빠른 처리로 좋은 성능

4. **기타 고성능 모델**
   - `deepseek-r1` / `deepseek-reasoner` - 강력한 추론 능력
   - `qwen3-max` - 과학적 맥락에서 우수
   - `grok-4` - 고급 언어 이해
...
consensus_results = interactive_consensus_annotation(
    ...
    consensus_model="claude-sonnet-4-5-20250929",  # 가장 우수한 모델 사용
...
single_model_results <- annotate_cell_types(
  ...
  model = "claude-sonnet-4-5-20250929",  # 단일 모델 지정
