...
![mLLMCelltype logo](../man/figures/logo.png)
...
![UMAP with Marginal Distributions](figures/png/fig4_marginal_distributions.png)
...
![Cell Types with Consensus Proportion](figures/png/fig7_cell_types_with_consensus.png)
...
![Uncertainty Metrics Violin Plots](figures/png/fig8_uncertainty_violins.png)
...
mLLMCelltype includes enhanced marker gene visualization functions that create publication-ready dotplots and heatmaps showing marker gene expression patterns across annotated cell types. These visualizations seamlessly integrate with the consensus annotation workflow:

- **Dotplots**: Classic visualization with marker genes vs cell types, showing both expression level and percentage
- **Heatmaps**: Comprehensive expression matrices with hierarchical clustering and consensus annotations
...
### Enhanced Bubble Plot Visualization

The bubble plot shows both the percentage of cells expressing each gene (bubble size) and the average expression level (color intensity):

```{r}
# Create enhanced bubble plot
bubble_result <- create_marker_bubble_plot(
  seurat_obj = seurat_obj,
  markers_df = pbmc_markers,
...
# Display the plot
print(bubble_result$plot)

# Save the plot
ggsave(
  filename = "marker_genes_bubble_plot.png",
  plot = bubble_result$plot,
  width = 12,
  height = 10,
  dpi = 300,
  bg = "white"
...
![Marker Gene Bubble Plot](figures/png/marker_bubble_plot.png)

This bubble plot visualization shows:

- **X-axis**: Cell types (from consensus annotation)
- **Y-axis**: Marker genes (ordered by expression level)
- **Bubble size**: Percentage of cells expressing the gene
- **Color intensity**: Average expression level (log2 scale)
...
png("marker_genes_heatmap.png", width = 1000, height = 800, res = 300, bg = "white")
...
![Marker Gene Heatmap](figures/png/marker_heatmap.png)

This heatmap visualization shows:

- **Rows**: Marker genes (hierarchically clustered)
- **Columns**: Cell types (from consensus annotation)
- **Color scale**: Scaled expression values (row-wise scaling)
- **Clustering**: Genes with similar expression patterns are grouped together

### Customizing Marker Gene Visualizations

Both visualization functions offer extensive customization options:

```{r}
# Bubble plot with different parameters
bubble_custom <- create_marker_bubble_plot(
  seurat_obj = seurat_obj,
  markers_df = pbmc_markers,
  consensus_results = consensus_results,
  top_n = 3,  # Show fewer genes per cell type
  title = "Top 3 Marker Genes by Cell Type",
  color_palette = "viridis"  # Different color palette
)

# Heatmap without gene clustering
png("marker_genes_heatmap_no_clustering.png", width = 1000, height = 800, res = 300, bg = "white")
heatmap_no_cluster <- create_marker_heatmap(
  seurat_obj = seurat_obj,
  markers_df = pbmc_markers,
  consensus_results = consensus_results,
  top_n = 4,
  title = "Marker Gene Expression (No Gene Clustering)",
  cluster_genes = FALSE,  # Disable gene clustering
  scale_method = "row",
  color_palette = viridis::inferno(100)  # Custom color palette
)
dev.off()
```

### Combining Marker Gene Visualizations with Cell Type Plots

Create a comprehensive figure combining cell type annotations with marker gene visualizations:
...
p_umap <- SCpubr::do_DimPlot(
  ...
  colors.use = custom_colors,
  font.size = 12,
  plot.title = "Cell Type Annotations"
) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
combined_marker_viz <- p_umap | bubble_plot$plot

# Add overall title
combined_marker_viz <- combined_marker_viz +
  plot_annotation(
    title = "Cell Type Annotation and Marker Gene Expression",
    subtitle = "Left: UMAP with cell type labels, Right: Marker gene bubble plot",
    theme = theme(
...
ggsave(
  "combined_celltype_markers.png",
  plot = combined_marker_viz,
  width = 20,
  height = 10,
  dpi = 300,
  bg = "white"
...
1. **Gene Selection**: Use `top_n = 3-5` for clarity; too many genes can make plots cluttered
2. **Color Palettes**:
   - Use `plasma` or `viridis` for bubble plots (good contrast)
   - Use `inferno` or custom palettes for heatmaps
   - Use blue-white-red for heatmaps (intuitive for expression)
3. **Scaling**: Row-wise scaling (`scale_method = "row"`) is recommended for heatmaps
4. **Dotplots**: Use appropriate dot size scaling and clear color gradients
