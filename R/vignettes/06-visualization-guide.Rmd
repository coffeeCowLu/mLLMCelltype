...
# This handles cases where cluster IDs don't match the indices in final_annotations
if (length(names(final_annotations)) > 0 && all(names(final_annotations) %in% cluster_ids)) {
  # If final_annotations has named entries matching cluster IDs
  annotation_map <- final_annotations[cluster_ids]
} else if (length(final_annotations) == length(cluster_ids)) {
  # If the number of annotations matches the number of clusters
  annotation_map <- final_annotations
  names(annotation_map) <- cluster_ids
} else {
  # Create a mapping based on available annotations
  annotation_map <- final_annotations[1:min(length(final_annotations), length(cluster_ids))]
  names(annotation_map) <- cluster_ids[1:length(annotation_map)]
  # Fill in missing annotations if needed
  if (length(annotation_map) < length(cluster_ids)) {
    for (i in (length(annotation_map)+1):length(cluster_ids)) {
      annotation_map[cluster_ids[i]] <- paste("Unknown", i)
    }
  }
}
