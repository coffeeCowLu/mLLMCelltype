...
seurat_obj$shannon_entropy <- as.numeric(plyr::mapvalues(
  x = as.character(Idents(seurat_obj)),
...
create_prediction_matrix <- function(consensus_results, models) {
  n_clusters <- length(consensus_results$final_consensus)
  ...
  for (i in 1:length(models)) {
    model <- models[i]
    predictions <- consensus_results$initial_results[[model]]
...
annotation_row <- data.frame(
  Consensus = consensus_results$final_consensus,
  Proportion = consensus_results$consensus_proportion,
  Entropy = consensus_results$shannon_entropy
)
