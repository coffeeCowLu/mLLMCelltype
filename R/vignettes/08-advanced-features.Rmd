...
# Approach 1: Use multi-model consensus
batch_consensus <- create_consensus(
  results = batch_results,  # Results from multiple models
  input = batch_affected_markers,
  tissue_name = "mouse brain",
  model = "claude-3-7-sonnet-20250219",
  api_key = Sys.getenv("ANTHROPIC_API_KEY"),
  discussion_threshold = 0.4  # Lower threshold to discuss more clusters
)
...
# Create consensus with lower discussion threshold
rare_cell_consensus <- create_consensus(
  results = rare_cell_results,
  input = rare_cell_markers,
  tissue_name = "human bone marrow",
  model = "claude-3-7-sonnet-20250219",
  api_key = api_keys[["anthropic"]],
